"""VD1 camera protocol implementation - HA-agnostic, stdlib-only.

Adapted from the standalone cli.py in the repo root: wakes the camera via
the Imou Cloud OpenAPI, keeps a DVRIP/37777 control connection alive
(eventManager.attach + A1 keepalive) so the camera doesn't fall back
asleep, and opens an HTTP/8086 realmonitor.xav video connection (HTTP
Digest auth, PLAY) whose DHAV-framed bytes get fanned out to any number
of consumers via Broadcaster.

Unlike cli.py (which runs its supervisors until the process is killed),
CameraConnection.close() must stop both supervisor threads within a
bounded time, since Home Assistant reloads/unloads config entries during
normal operation. That's the one real behavioral difference from cli.py:
every retry/backoff sleep is a stop_event.wait() instead of a bare
time.sleep(), and each supervisor publishes its currently-live socket
back onto the CameraConnection so close() can close it out from under a
thread blocked in recv() - is_set() alone can't interrupt a blocking
read, only closing the socket can.

This module imports nothing from `homeassistant`, so it can be run and
smoke-tested standalone exactly like cli.py.
"""
...
def strip_transport_framing(buffer):
    ...
    genuine 'HTTP/1.1 ...' reply to our own ping (see http_keepalive) can
    be interleaved into the same byte stream and is detected and
    discarded here so it never reaches the caller as bogus frame data."""
    parts = []
    pos = 0
...
def listen_events(sock, session, stop_event, on_event=None):
    """Attach to the camera's eventManager (codes=All) and, for every
    pushed event, log it at debug level and (if given) call
    on_event(body) with the raw event body. Runs until stop_event is
    set - but since the thread is blocked in recv_exact()/sock.recv(),
    stop_event alone can't interrupt an in-flight read; the caller must
    also close `sock` from another thread to unblock it. The is_set()
    check here only prevents looping back into another blocking read
    once that happens."""
    payload = json.dumps(
        ...
        if b'"result"' not in body:  # skip the attach ack itself
            ...
            if on_event is not None:
                try:
                    ...
                    _LOGGER.exception("imou_vd1: event subscriber raised")
...
# supervisor loops until stop_event is set: connect (bare attempt,
# wake-and-retry once on failure), start the keepalive thread, run the
# reader (blocking), then on any failure stop the keepalive and
# reconnect.
# ============================================================

def connect_dvrip_or_wake(conn, stop_event):
    try:
        return open_dvrip_connection(conn.host, conn.dvrip_port, conn.username, conn.password)
    except Exception:
        wake_camera(conn.imou_app_id, conn.imou_app_secret, conn.imou_device_id, conn.imou_data_center)
        stop_event.wait(WAKE_SETTLE_DELAY)
        return open_dvrip_connection(conn.host, conn.dvrip_port, conn.username, conn.password)


def connect_http_or_wake(conn, stop_event):
    try:
        return open_http_connection(
            conn.host, conn.http_port, conn.username, conn.password, conn.channel, conn.stream
        )
    except Exception:
        wake_camera(conn.imou_app_id, conn.imou_app_secret, conn.imou_device_id, conn.imou_data_center)
        stop_event.wait(WAKE_SETTLE_DELAY)
...
def dvrip_supervisor(conn, stop_event):
    while not stop_event.is_set():
        try:
            sock, session = connect_dvrip_or_wake(conn, stop_event)
        except Exception:
            if stop_event.wait(RETRY_DELAY):
                break
            continue

        with conn._sock_lock:
            conn._dvrip_sock = sock

        keepalive_stop = threading.Event()
        threading.Thread(
            target=dvrip_keepalive,
            args=(sock, keepalive_stop),
            daemon=True,
        ).start()

        try:
            listen_events(sock, session, stop_event, on_event=conn._dispatch_event)
        except Exception:
            pass
        ...
        sock.close()
        with conn._sock_lock:
            if conn._dvrip_sock is sock:
                conn._dvrip_sock = None

        if stop_event.wait(RETRY_DELAY):
            break


def http_supervisor(conn, stop_event):
    """Keeps the HTTP/8086 video channel connected until stop_event is
    set: connect (bare attempt, wake-and-retry once on failure), start
    the keepalive thread, then yield (sock, buffer) for the caller to
    read from. Resumes once the caller's `for` loop advances - after
    it's done with, or given up on, that connection - tearing down the
    keepalive and reconnecting before yielding the next one."""
    while not stop_event.is_set():
        try:
            sock, buffer, ping_path = connect_http_or_wake(conn, stop_event)
        except Exception:
            if stop_event.wait(RETRY_DELAY):
                break
            continue
        ...
        with conn._sock_lock:
            conn._http_sock = sock

        keepalive_stop = threading.Event()
        threading.Thread(
            target=http_keepalive,
            args=(sock, conn.host, conn.http_port, ping_path, keepalive_stop),
            daemon=True,
        ).start()
        ...
        keepalive_stop.set()
        with conn._sock_lock:
            if conn._http_sock is sock:
                conn._http_sock = None
...
class Broadcaster:
    ...
    out to every currently connected client instead.
    ...
    is what starts/stops streaming."""
    ...
    def add_client(self, wfile):
        with self._lock:
            ...
            self.streaming.set()
        _LOGGER.info("Broadcaster: client connected")

    def remove_client(self, wfile):
        with self._lock:
            if wfile in self._clients:
                self._clients.remove(wfile)
                _LOGGER.info("Broadcaster: client disconnected")
...
def video_worker(conn, broadcaster, stop_event):
    ...
    broadcaster.streaming, which Broadcaster itself sets/clears as
    clients come and go."""
    for sock, buffer in http_supervisor(conn, stop_event):
        try:
...
class CameraConnection:
    """Owns one VD1 camera's DVRIP control channel and HTTP/8086 video
    channel, each run by its own supervisor on a daemon thread, plus a
    Broadcaster fanning the video out to any number of consumers and a
    subscriber registry fanning DVRIP events out to any number of
    callbacks. connect()/close() bound the lifetime of both supervisor
    threads; wake() is a one-shot best-effort cloud wake-up."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dvrip_port = dvrip_port
        self.http_port = http_port
        self.channel = channel
        self.stream = stream
        self.imou_app_id = imou_app_id
        self.imou_app_secret = imou_app_secret
        self.imou_device_id = imou_device_id
        self.imou_data_center = imou_data_center
        ...
        self._dvrip_sock = None
        self._http_sock = None

        self._subscribers_lock = threading.Lock()
        self._event_subscribers: dict[int, tuple] = {}
        self._next_token = itertools.count(1)

    def wake(self):
        wake_camera(self.imou_app_id, self.imou_app_secret, self.imou_device_id, self.imou_data_center)

    def connect(self):
        self._stop_event.clear()

        dvrip_thread = threading.Thread(
            target=dvrip_supervisor,
            args=(self, self._stop_event),
            daemon=True,
            name=f"imou_vd1-dvrip-{self.host}",
        )
        video_thread = threading.Thread(
            target=video_worker,
            args=(self, self.broadcaster, self._stop_event),
            daemon=True,
            name=f"imou_vd1-video-{self.host}",
        )

        dvrip_thread.start()
        video_thread.start()

        self._threads = [dvrip_thread, video_thread]

    def close(self, timeout=5.0):
        self._stop_event.set()

        with self._sock_lock:
            sockets = [self._dvrip_sock, self._http_sock]
        ...
        for thread in self._threads:
            ...
            if thread.is_alive():
                _LOGGER.warning("imou_vd1: %s did not stop within %.1fs", thread.name, timeout)

        self._threads = []

    def open_stream(self):
        """Returns the Broadcaster - callers attach with add_client()/
        remove_client(), same as any other write()/flush() sink."""
    ...
    def _dispatch_event(self, body):
        ...
        for callback, codes in subscribers:
            if codes is not None and not any(code.encode() in body for code in codes):
                continue
            try:
                callback(body)
            except Exception:
    ...
    def subscribe_events(self, callback, codes=None):
        token = next(self._next_token)
        with self._subscribers_lock:
            self._event_subscribers[token] = (callback, codes)
        return token
